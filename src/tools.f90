...
module tools
  ...
  character(len=*), parameter :: rezfile = "ireztart"
...
contains
  ...
  subroutine restart(ux1,uy1,uz1,dux1,duy1,duz1,ep1,pp3,phi1,dphi1,px1,py1,pz1,rho1,drho1,mu1,iresflg)
    ...
    if (iresflg==1) then !write
	   ...
	   ! Tolstoy Debbarma
	   rename_stepp = (ilast-ifirst+1)/num_renamess
       ...
       if (validation_restart) then
          if (validate_restart(resfile_old, resfile)) then
			 if ((itime - ifirst) > 0 .and. (itime - ilast) <= 0 .and. mod(itime-ifirst+1,rename_stepp) == zero) then
				call rename_with_itime(resfile_old,rezfile,itime-icheckpoint)			! Tolstoy Debbarma
			 else
  ...
  subroutine rename_with_itime(oldname, newname, itime, opt_rank)
	...
	character(len=20) :: itime_str  ! Temporary variable for integer to string conversion
	character(len=256) :: newname_base
    ...
    if (nrank == exe_rank) then
       ...
       if (exist) then
		  ...
		  write(itime_str, '(I0)') itime
		  newname_base = trim(newname) // '_itime' // trim(itime_str)			! modification by Tolstoy
          
		  cmd = "mv "//oldname_ext//" "//newname_base
          call execute_command_line(cmd)
       end if
    end if
  ...
  end subroutine rename_with_itime
