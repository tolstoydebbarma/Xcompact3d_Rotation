...
contains
  ...
  SUBROUTINE solve_poisson(pp3, px1, py1, pz1, rho1, ux1, uy1, uz1, ep1, drho1, divu3)
	...
	if (nrank==0) then
		write(*,*) 'I am in solve_poisson before divergence'
	endif
    ...
    do while(.not.converged)
       if (ivarcoeff) then
          ...
          IF (.NOT.converged) THEN
			 ...
			 if (nrank==0) then
				write(*,*) 'I am in 2nd solve_poisson before calc_varcoeff_rhs'
			 endif
  ...
  subroutine divergence (pp3,rho1,ux1,uy1,uz1,ep1,drho1,divu3,nlock)
	...
	real(mytype), parameter :: tol=1e-6
	
	! modification
	real(mytype) :: ym,zm,r,vel
    ...
    if (iibm.eq.0) then
       ...
       tc1(:,:,:) = uz1(:,:,:)
	    ...
	    do k = 1, xsize(3)
			zm=dz*real(xstart(3)-1+k-1,mytype)-zlz/two
			do j = 1, xsize(2)
				ym=real(j+xstart(2)-1-1,mytype)*dy-yly/two
				r=sqrt(ym*ym+zm*zm)
				do i = 1, xsize(1)
					if ((ep1(i,j,k).eq.one) .and. ((r-rai).le.tol)) then
						ta1(i,j,k)=zero
						tb1(i,j,k)=(-vel*(real(k+xstart(3)-1-1)*dz-(zlz/two)))
						tc1(i,j,k)=(real(j+xstart(2)-1-1)*dy-(yly/two))
					else
						ta1(i,j,k) = (one - ep1(i,j,k)) * ux1(i,j,k) + ep1(i,j,k)*ubcx
						tb1(i,j,k) = (one - ep1(i,j,k)) * uy1(i,j,k) + ep1(i,j,k)*ubcy
						tc1(i,j,k) = (one - ep1(i,j,k)) * uz1(i,j,k) + ep1(i,j,k)*ubcz
					endif
				enddo
			enddo
		enddo
  ...
  subroutine pre_correc(ux,uy,uz,ep)
	...
	! +================================================================================
	! This is the velocity correction for Pipe flow TD
	if ((itype.eq.itype_pipe_TD).and.(nclx1==0.and.nclxn==0)) then

        !Correct bulk velocity
        call pipe_bulk_TD(ux,uy,uz,ep)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !!  subroutine: pipe_bulk_TD
  !!      AUTHOR: Tolstoy Debbarma
  !! DESCRIPTION: Correction of pipe's bulk velocity (constant 
  !!              flow rate) as per Rodrigo
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !********************************************************************
  !
  subroutine pipe_bulk_TD(ux,uy,uz,ep)
  !
  !********************************************************************
  ...
  !********************************************************************
    ...
    if (local_io_unit.eq.-1 .and. nrank.eq.0) then
       open(newunit=local_io_unit,file='Ub.dat',status='unknown', position='append', action='write')
    endif
    ...
    if (nrank==0) then
       ...
       write(local_io_unit,*) real((itime-1)*dt,mytype), qm, (ub_constant-qm) !vel, write pressure drop
    endif
