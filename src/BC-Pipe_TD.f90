...
module pipe_TD
  ...
  PUBLIC :: geomcomplex_pipe_TD, init_pipe_TD
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !********************************************************************
  !
  subroutine geomcomplex_pipe_TD(epsi,nxi,nxf,ny,nyi,nyf,nzi,nzf,dx,yp,dz,remp)
  !
  !********************************************************************
  ...
  end subroutine geomcomplex_pipe_TD
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !********************************************************************
  !
  subroutine init_pipe_TD (ux1,uy1,uz1,ep1,phi1)
  !
  !********************************************************************
	...
	real(mytype), parameter :: del_p=0.05
	real(mytype) :: uth_p,uy_p,uz_p
    ...
    do k=1,xsize(3)
        ...
        do j=1,xsize(2)
			...
			uth_p = omega_surf*r	!*(1d0 - erf((r-0.45)/del_p))
			
			
			 if (r .ne. 0 .and. r.le.rai) then
            ...
            !Poiseuille flow
            bxx1(j,k)=two*(one-(ym**two+zm**two)/(rai**two))				!two-8*r**two		!two*(one-(ym**two+zm**two)/(rai**two))
            bxy1(j,k)=uy_p	!zero
            bxz1(j,k)=uz_p	!zero
