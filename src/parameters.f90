...
subroutine parameter(input_i3d)
  ...
  NAMELIST /ibmstuff/ cex,cey,cez,ra,rai,rao,nobjmax,nraf,nvol,iforces, npif, izap, ianal, imove, thickness, chord, omega ,ubcx,ubcy,ubcz, omega_surf, rads, c_air
  ...
  NAMELIST /Swirling/ alp_p, del_p, del_cf, S_p, len_nz
  
  NAMELIST /SwirlingNoNozzle/ alp_nonz, del_nonz, del_cf_nonz, S_nonz, rao_nonz
  ...
  if (itype.eq.itype_jet_TD) then
	 read(10, nml=Swirling); rewind(10)
  endif
  if (itype.eq.itype_jet_nonozzle) then
	 read(10, nml=SwirlingNoNozzle); rewind(10)
  endif
  
  close(10)
  
  convModulo_p = nint(convT/dt)
  ...
  ! Log-output
  !###########################################################################
  if (nrank==0) call system('mkdir data out probes statistics 2> /dev/null')
  ...
  if (nrank==0) then
	 ...
	 elseif (itype.eq.itype_pipe_TD) then
        print *,'Simulating pipe TD'
	 elseif (itype.eq.itype_jet_TD) then
        print *,'Simulating Jet TD'
	 elseif (itype.eq.itype_jet_nonozzle) then
        print *,'Simulating Jet No Nozzle'
	 elseif (itype.eq.itype_chk_buffer) then
        print *,'Simulating Check Buffer'
