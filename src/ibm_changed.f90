     ...
     do j=1,xsize(2)
        if(nobjx(j,k).ne.0)then
           ...
           do i=1,nobjx(j,k)          
			  ...
			  !modification starts
              if (lind .eq. 0.0_mytype .or. lind .eq. zero) then
                bcimp1=lind
				
              else if (lind.eq.1.0_mytype) then
                bcimp1= -vel*(real(k+xstart(3)-1-1)*dz-(zlz/two))
                
              else if (lind.eq.2.0_mytype) then   
                bcimp1= vel*(real(j+xstart(2)-1-1)*dy-(yly/two))
				 
              else if (lind.eq.3.0_mytype) then
                bcimp1= vel*vel*(real(k+xstart(3)-1-1)*dz-(zlz/two))**two
				 
              else if (lind.eq.4.0_mytype) then  
                bcimp1= vel*vel*(real(j+xstart(2)-1-1)*dy-(yly/two))**two
				 
              else if (lind.eq.5.0_mytype) then
                bcimp1= -vel*vel*(real(k+xstart(3)-1-1)*dz-(zlz/two))*(real(j+xstart(2)-1-1)*dy-(yly/two))
              ...
              if (lind .eq. 0.0_mytype .or. lind .eq. zero) then
                ...
                bcimp2= -vel*(real(k+xstart(3)-1-1)*dz-(zlz/two))
                
              else if (lind.eq.2.0_mytype) then   
                bcimp2= vel*(real(j+xstart(2)-1-1)*dy-(yly/two))
				 
              else if (lind.eq.3.0_mytype) then
                bcimp2= vel*vel*(real(k+xstart(3)-1-1)*dz-(zlz/two))**two
				 
              else if (lind.eq.4.0_mytype) then  
                bcimp2= vel*vel*(real(j+xstart(2)-1-1)*dy-(yly/two))**two
				 
              else if (lind.eq.5.0_mytype) then
                bcimp2= -vel*vel*(real(k+xstart(3)-1-1)*dz-(zlz/two))*((j+xstart(2)-1-1)*dy-(yly/two))
        ...
        if(nobjy(i,k).ne.0)then
           ...
           do j=1,nobjy(i,k)          
              ...
              if (lind .eq. 0.0_mytype .or. lind .eq. zero) then
				...
				temp=sqrt(rai**2.0-(xa(ia)-(yly/two))**two)
				 if (real(k+ystart(3)-1-1)*dz.gt.(zlz/two)) then     
                  bcimp1= -vel*(temp)
                 else
                  bcimp1= vel*(temp)
                 endif 
                ...
                bcimp1= vel*(xa(ia)-(yly/two))
				 
              else if (lind.eq.3.0_mytype) then
                temp=sqrt(rai**2.0-(xa(ia)-(yly/two))**2.0)
				bcimp1= vel*vel*(temp)**two 
				 
              else if (lind.eq.4.0_mytype) then
                 bcimp1= vel*vel*(xa(ia)-(yly/two))**two
				 
              else if (lind.eq.5.0_mytype) then
                 temp=sqrt(rai**2.0-(xa(ia)-(yly/two))**2.0)
				 if (real(k+ystart(3)-1-1)*dz.gt.(zlz/two)) then     
                  bcimp1= -vel*vel*(temp)*(xa(ia)-(yly/two))
                 else
                  bcimp1= vel*vel*(temp)*(xa(ia)-(yly/two))
                 endif 
              ...
              if (lind .eq. 0.0_mytype .or. lind .eq. zero) then
                ...
                bcimp2= vel*(xa(ia)-(yly/two))
				 
              else if (lind.eq.3.0_mytype) then
                temp=sqrt(rai**2.0-(xa(ia)-(yly/two))**2.0)
				bcimp2= vel*vel*(temp)**two 
				 
              else if (lind.eq.4.0_mytype) then
                 bcimp2= vel*vel*(xa(ia)-(yly/two))**two
				 
              else if (lind.eq.5.0_mytype) then
                 temp=sqrt(rai**2.0-(xa(ia)-(yly/two))**2.0)
				 if (real(k+ystart(3)-1-1)*dz.gt.(zlz/two)) then     
                  bcimp2= -vel*vel*(temp)*(xa(ia)-(yly/two))
                 else
                  bcimp2= vel*vel*(temp)*(xa(ia)-(yly/two))
                 endif 
...
!                 call analitic_z(i,zi(k,i,j),ana_resi,j) ! Calculate the position of BC analytically
              ...
              if (lind .eq. 0.0_mytype .or. lind .eq. zero) then
                ...
                bcimp1=-vel*(xa(ia)-(zlz/two))
				
              else if (lind.eq.2.0_mytype) then     
                temp=sqrt(ra**2.0-(xa(ia)-(zlz/two))**2.0)
                 if (real(j+zstart(2)-1-1)*dy.gt.(yly/two)) then
					bcimp1= vel*(temp)
                 else
					bcimp1= -vel*(temp)
                 endif
                ...
                bcimp1= vel*vel*(xa(ia)-(zlz/two))**2.0   
				
              else if (lind.eq.4.0_mytype) then    
				temp=sqrt(ra**2.0-(xa(ia)-(zlz/two))**2.0)			  
                bcimp1= vel*vel*(temp)**2.0
				
              else if (lind.eq.5.0_mytype) then
                temp=sqrt(ra**2.0-(xa(ia)-(zlz/two))**2.0)
                 if (real(j+zstart(2)-1-1)*dy.gt.(yly/two)) then
					bcimp1= -vel*vel*(temp)*(xa(ia)-(zlz/two))
                 else
					bcimp1= vel*vel*(temp)*(xa(ia)-(zlz/two))
                 endif
              endif
			  ...
			  if (lind .eq. 0.0_mytype .or. lind .eq. zero) then
                ...
                bcimp2= vel*vel*(xa(ia)-(zlz/two))**2.0   
				
              else if (lind.eq.4.0_mytype) then    
				temp=sqrt(ra**2.0-(xa(ia)-(zlz/two))**2.0)			  
                bcimp2= vel*vel*(temp)**2.0
				
              else if (lind.eq.5.0_mytype) then
                temp=sqrt(ra**2.0-(xa(ia)-(zlz/two))**2.0)
                 if (real(j+zstart(2)-1-1)*dy.gt.(yly/two)) then
					bcimp2= -vel*vel*(temp)*(xa(ia)-(zlz/two))
                 else
					bcimp2= vel*vel*(temp)*(xa(ia)-(zlz/two))
                 endif
              endif
